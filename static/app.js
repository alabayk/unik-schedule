...
function showUpdated(value,cached=false){const el=$('#updated');if(!el)return;if(!value){el.hidden=true;return}const d=new Date(value);el.hidden=false;el.textContent=(cached?'Сохранено ':'Обновлено ')+d.toLocaleDateString('ru-RU',{day:'numeric',month:'short'})+' в '+d.toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}
