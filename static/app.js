...
function moveDay(step){showPast=false;selected+=step;if(selected>6){monday=addDays(monday,7);selected=0;load();return}if(selected<0){monday=addDays(monday,-7);selected=6;load();return}renderDays();renderLessons()}let swipeX=0,swipeY=0;const lessonArea=$('#lessons');lessonArea.addEventListener('touchstart',e=>{swipeX=e.touches[0].clientX;swipeY=e.touches[0].clientY},{passive:true});lessonArea.addEventListener('touchend',e=>{const dx=e.changedTouches[0].clientX-swipeX,dy=e.changedTouches[0].clientY-swipeY;if(Math.abs(dx)>60&&Math.abs(dx)>Math.abs(dy)*1.4)moveDay(dx<0?1:-1)},{passive:true});
